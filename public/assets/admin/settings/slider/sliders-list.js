...
$(document).ready(function () {
  ...
  var table = $('#tblSlider').DataTable({
    lengthChange: false,
    dom: 'Bfrtip',
    buttons: ['copy', 'excel', 'pdf', 'csv' , 'colvis'],

    ajax: {
        "url": "/admin/slider/datatable",
        "dataSrc": ""
    },
    columns: [
      { data : 'id'},
      ...
      { render : function(data, type, row , full) {
        // console.log(row)
        if (row.status == '1') {
            return `<a href="javascript:void(0)" onclick="change_status('`+row.id+`')" class="badge badge-pill badge-success">active</a>`
        }else{
            return 'deactive'
        }
         },
     ...
     { render : function(data, type, row , full) {
       // console.log(row)
         return `
         <div class="glyph">
             <a href="/admin/slider/`+row.id+`/edit"> <i class="mdi mdi-home-edit"></i> </a>
